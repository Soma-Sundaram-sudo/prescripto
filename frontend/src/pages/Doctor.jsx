  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5 '>
        ...
        <div className={` flex-col text-gray-600 text-sm gap-4 ${showfilter ? "flex":"hidden"}`}>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterdoc.map((item,index)=>(
                <div onClick={()=>navigate(`/appointment/${item._id}`)} key={index} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
                    ...
                    <div className='p-4'>
                    <div className={`flex items-center gap-2 text-sm text-center ${item.avaialable ?  "text-green-500":"text-red-500"} `}>
                        <p className={`w-2 h-2 ${item.avaialable ? "bg-green-200":"bg-red-200"}  rounded-full`}></p><p>{item.avaialable ? "Available":"Not Available"}</p>
                    </div>
